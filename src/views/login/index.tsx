...
// 导入登录和注册接口
import { getQrcode, login, registry, getQrcodeStatus } from '@/service/auth';
    ...
    const handleQrCodeLogin = async () => {
        try {
            ...
            const timer = setInterval(async () => {
                await getQrcodeStatus(data!.qrcodeId);
            }, 1500);
            setTimer(timer);
        } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-200 flex items-center justify-center p-4 relative">
            ...
            <div className="relative z-10 w-full max-w-md bg-white/30 backdrop-blur-lg rounded-2xl shadow-xl p-8 space-y-6 border border-white/20">
                <div className="text-center">
                    <h1 className="text-3xl font-bold text-gray-900">
                        {isRegisterForm ? '注册新账户' : '欢迎回来'}
                    </h1>
                    <p className="text-gray-500 mt-2">
                        {isRegisterForm ? '请输入您的注册信息' : '请输入您的登录信息'}
                    </p>
                </div>

                <Tabs defaultValue="account" className="w-full" onValueChange={ onTabsValueChange}>
                    <TabsList className="grid w-full grid-cols-2">
                        <TabsTrigger value="account">账号密码登录</TabsTrigger>
                        <TabsTrigger value="qrcode">二维码登录</TabsTrigger>
                    </TabsList>
            ...
            LinkedIn
                ...
                <div className="absolute top-12 right-8 w-16 h-16 bg-pink-400/30 rounded-full blur-xl" />
                <div className="absolute bottom-8 left-12 w-20 h-20 bg-green-400/30 rounded-full blur-xl" />
                <div className="absolute top-24 left-24 w-12 h-12 bg-yellow-400/30 rounded-full blur-xl" />
                <div className="absolute bottom-24 right-24 w-16 h-16 bg-orange-400/30 rounded-full blur-xl" />
            </div>
        </div>
    );
}
